# Money Tracker Documentation

Welcome to the Money Tracker documentation! This comprehensive guide will help you get started, use the application effectively, and contribute to the project.

## Current System

If you are onboarding to the current production architecture, start here first.

- [Current System Overview](./current-system/README.md)
- [Component Map](./current-system/component-map.md)
- [Chat Router, Orchestration, Prompts, And Tools](./current-system/chat-router.md)
- [Runtime Pipelines](./current-system/pipelines.md)
- [API Surface](./current-system/api-surface.md)
- [Memory And State](./current-system/memory-and-state.md)

These files describe the active code paths as of March 6, 2026. Where they conflict with older generic docs, prefer the current-system set.

## 📚 Documentation Sections

### For Users

- **[Getting Started](./getting-started/)** - Installation, configuration, and first-time setup
  - [Installation Guide](./getting-started/installation.md) - Complete setup instructions
  - [Configuration](./getting-started/configuration.md) - Environment variables and settings
  - [First-Time Setup](./getting-started/first-time-setup.md) - User walkthrough
  - [Quick Tour](./getting-started/quick-tour.md) - Feature overview

- **[User Guide](./user-guide/)** - Complete feature documentation and tutorials
  - [Managing Accounts](./user-guide/accounts.md)
  - [Creating Transactions](./user-guide/transactions.md)
  - [Budget Planning](./user-guide/budgets.md)
  - [Savings Goals](./user-guide/goals.md)
  - [Recurring Transactions](./user-guide/recurring-transactions.md)
  - [Split Bills & Settlements](./user-guide/split-bills.md)
  - [Transaction Templates](./user-guide/templates.md)
  - [Spending Watchlists](./user-guide/watchlists.md)
  - [Receipt Management](./user-guide/receipts.md)
  - [Analytics & Reports](./user-guide/analytics.md)
  - [AI Features (Saathi)](./user-guide/ai-features.md)
  - [Export & Import](./user-guide/export-import.md)

- **[Troubleshooting](./troubleshooting/)** - Common issues and solutions
  - [Common Issues](./troubleshooting/common-issues.md)
  - [Database Problems](./troubleshooting/database-issues.md)
  - [Authentication Issues](./troubleshooting/authentication-issues.md)
  - [Build Errors](./troubleshooting/build-issues.md)

### For Developers

- **[Architecture](./architecture/)** - System design and technical overview
  - [Architecture Overview](./architecture/README.md)
  - [Tech Stack](./architecture/tech-stack.md)
  - [Project Structure](./architecture/project-structure.md)
  - [Database Schema](./architecture/database-schema.md)
  - [Data Flow](./architecture/data-flow.md)
  - [State Management](./architecture/state-management.md)
  - [Authentication](./architecture/authentication.md)
  - [API Design](./architecture/api-design.md)
  - [AI Integration](./architecture/ai-integration.md)

- **[API Reference](./api/)** - Complete REST API documentation
  - [API Overview](./api/README.md)
  - [Authentication](./api/authentication.md)
  - [Accounts](./api/accounts.md)
  - [Transactions](./api/transactions.md)
  - [Budgets](./api/budgets.md)
  - [Categories](./api/categories.md)
  - [Parties](./api/parties.md)
  - [Goals](./api/goals.md)
  - [Watchlists](./api/watchlists.md)
  - [Recurring Transactions](./api/recurring.md)
  - [Notifications](./api/notifications.md)
  - [Insights](./api/insights.md)
  - [AI Chatbot](./api/chat.md)
  - [Templates](./api/templates.md)
  - [Settlements](./api/settlements.md)
  - [Receipts](./api/receipts.md)
  - [Error Handling](./api/error-handling.md)

- **[Development Guide](./development/)** - Local development and best practices
  - [Local Development](./development/local-development.md)
  - [Database Setup](./development/database-setup.md)
  - [Coding Standards](./development/coding-standards.md)
  - [Component Patterns](./development/component-patterns.md)
  - [Adding Features](./development/adding-features.md)
  - [Testing Guidelines](./development/testing.md)
  - [Debugging](./development/debugging.md)
  - [Performance](./development/performance.md)

- **[Deployment](./deployment/)** - Production deployment guides
  - [Vercel Deployment](./deployment/vercel.md)
  - [Environment Variables](./deployment/environment-variables.md)
  - [Database Migration](./deployment/database-migration.md)
  - [Monitoring](./deployment/monitoring.md)

### For Contributors

- **[Contributing](./contributing/)** - How to contribute to this project
  - [Contribution Guidelines](./contributing/README.md)
  - [Code of Conduct](./contributing/code-of-conduct.md)
  - [Pull Request Process](./contributing/pull-request-template.md)
  - [Issue Templates](./contributing/issue-templates.md)
  - [Development Workflow](./contributing/development-workflow.md)

- **[Changelog](./changelog/)** - Feature roadmap and completed work
  - [Roadmap](./changelog/roadmap.md)
  - [Completed Features](./changelog/completed-features.md)

### Diagrams

- **[Architecture Diagrams](./diagrams/)** - Visual representations
  - [Architecture Overview](./diagrams/README.md)
  - Database ERD
  - Authentication Flow
  - Transaction Flow
  - Component Hierarchy

---

## 🚀 Quick Links

**New to Money Tracker?**
- Start with [Installation Guide](./getting-started/installation.md)
- Follow [First-Time Setup](./getting-started/first-time-setup.md)
- Take a [Quick Tour](./getting-started/quick-tour.md)

**Using the Application?**
- Browse the [User Guide](./user-guide/)
- Check [Troubleshooting](./troubleshooting/common-issues.md) for issues

**Developing?**
- Review [Architecture Overview](./architecture/README.md)
- Read [API Documentation](./api/README.md)
- Follow [Development Guide](./development/local-development.md)

**Contributing?**
- Read [Contributing Guidelines](./contributing/README.md)
- Check [Development Workflow](./contributing/development-workflow.md)
- Review [Roadmap](./changelog/roadmap.md)

---

## 📦 Tech Stack Overview

- **Frontend**: Next.js 15, React 19, TypeScript, Tailwind CSS
- **Backend**: Next.js API Routes, Prisma ORM
- **Database**: PostgreSQL (Neon)
- **Authentication**: NextAuth.js with Google OAuth
- **AI**: Google Gemini 2.0 Flash
- **UI Components**: Radix UI
- **Charts**: Recharts
- **State**: React Context API

---

## 🎯 Key Features

- **Account Management**: Track multiple bank accounts, credit cards, and savings
- **Transaction Tracking**: Record income and expenses with categories and tags
- **Budget Planning**: Create and monitor budgets with alerts and sub-categories
- **Savings Goals**: Set and track progress toward financial goals
- **Recurring Transactions**: Automate recurring bills and income
- **Split Bills**: Track shared expenses and settlements
- **AI Assistant (Saathi)**: Get personalized financial insights and advice
- **Receipt Management**: Attach receipts to transactions
- **Analytics**: Visualize spending patterns and trends
- **Export/Import**: Backup and restore data in JSON/CSV formats

---

## 📖 Documentation Standards

All documentation in this project follows these standards:
- **Markdown Format**: Standard GitHub-flavored markdown
- **Code Examples**: Working, tested examples included
- **Audience Tags**: Each doc indicates intended audience
- **Last Updated**: Date stamps for version tracking
- **Cross-References**: Links between related documentation
- **Practical Examples**: Real-world use cases included

---

## 🆘 Need Help?

- **Issues**: Check [Common Issues](./troubleshooting/common-issues.md)
- **Questions**: Review relevant [User Guide](./user-guide/) sections
- **Contributing**: See [Contributing Guidelines](./contributing/README.md)

---

## 📄 License

[Add your license information here]

---

*Last Updated: February 8, 2026*
